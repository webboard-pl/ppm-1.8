...
function upgrade3_dbchanges()
{
	...
	if($errors)
	{
		$output->print_contents($contents."<p><span style=\"color: red\">Aby wykonać ten krok musisz naprawić poniższe błędy:</span></p>$errors");
...
function upgrade3_convertattachments()
{
	...
	if($cnt['attachrem'] != 0)
	{
		...
		$startat = $startat+$app;
		$contents .= "<p><input type=\"hidden\" name=\"attachmentspage\" value=\"$app\" /><input type=\"hidden\" name=\"attachmentstart\" value=\"$startat\" />Zakończono. Kliknij dalej, aby przejść do kolejnej paczki załączników.</p>";
	}
	else
	{
		if($db->field_exists("donecon", "attachments"))
