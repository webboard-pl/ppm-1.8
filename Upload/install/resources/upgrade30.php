...
/**
 * Upgrade Script: 1.6.14, 1.6.15 lub 1.6.16
 */
...
function upgrade30_dbchanges()
{
	...
	global $footer_extra;
	$footer_extra = "<script type=\"text/javascript\">$(document).ready(function() { var button = $('.submit_button'); if(button) { button.val('Automatyczne przekierowanie...'); button.prop('disabled', true); button.css('color', '#aaa'); button.css('border-color', '#aaa'); document.forms[0].submit(); } });</script>";
...
function upgrade30_dbchanges6()
{
	...
	$db->insert_query("questions", array('question' => 'Jaki jest wynik działania 2+2?', 'answer' => '4\ncztery', 'active' => '1'));

	$query = $db->simple_select("attachtypes", "COUNT(*) as numexists", "extension='psd'");
	if($db->fetch_field($query, "numexists") == 0)
	{
		$db->insert_query("attachtypes", array('name' => "Plik Adobe Photoshop", 'mimetype' => 'application/x-photoshop', 'extension' => "psd", 'maxsize' => '1024', 'icon' => 'images/attachtypes/psd.png'));
	}
	...
	echo "<p>Aktualizowanie uprawnień grup...</p>";
	$groups = array();
...
function upgrade30_threadcount()
{
	...
	$output->print_header("Zliczanie liczby wątków użytkowników");
	...
	echo "<p>Zliczanie wątków użytkowników od #{$lower} do #{$upper} (Łącznie {$cnt['usercount']})</p>";
	flush();
	...
	if($remaining && $threadnum)
	{
		...
		$contents = "<p><input type=\"hidden\" name=\"theadspage\" value=\"$threads\" /><input type=\"hidden\" name=\"threadstart\" value=\"$startat\" />Done. Naciśnij przycisk Dalej, aby przejść do kolejnego zestawu użytkowników.</p>";
	}
	else
	{
		$nextact = "30_dbchanges_optimize1";
		$contents = "<p>Zakończono</p><p>Liczby wątków utworzonych przez poszczególnych użytkowników zostały ustalone. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
...
function upgrade30_dbchanges_optimize1()
{
	...
	$output->print_header("Wykonywanie optymalizacji");

	echo "<p>Trwa wykonywanie wymaganych optymalizacji bazy danych...</p>";
	flush();
...
function upgrade30_dbchanges_optimize2()
{
	...
	echo "<p>Trwa wykonywanie wymaganych optymalizacji bazy danych...</p>";
	echo "<p>Dodawanie indeksów do tabel...</p>";
	flush();
	...
	echo "<p>Usuwanie starych indeksów z tabel...</p>";
...
function upgrade30_dbchanges_optimize3()
{
	...
	{
		echo "<p>{$table}: Zmiana typu kolumny na tinyint</p>";
		$change_column = array();
...
function upgrade30_dbchanges_optimize4()
{
	...
	{
		echo "<p>{$table}: Zmiana typu kolumny na int</p>";
		$change_column = array();
		foreach($columns as $column)
		{
			if($db->type == "pgsql")
...
function upgrade30_dbchanges_smilies()
{
	...
	$output->print_header("Aktualizacja emotikon");

	echo "<p>Trwa wykonywanie aktualizacji emotikon w bazie danych...</p>";
	flush();
...
function upgrade30_dbchanges_ip()
{
	...
	$output->print_header("Konwersja IP");
	...
	{
		case 8:
			echo "<p>Dodawanie indeksów w bazie (3/3)...</p>";
			flush();
			...
			break;
		case 7:
			echo "<p>Dodawanie indeksów w bazie (2/3)...</p>";
			flush();
			...
			break;
		case 6:
			echo "<p>Dodawanie indeksów w bazie (1/3)...</p>";
			flush();
		...
		case 5:
			...
			{
				case 7:
					echo "<p>Konwersja adresów IP użytkowników...</p>";
					flush();
			...
			echo "<p>Konwersja adresów IP od {$lower} do {$upper} (Łącznie {$cnt['ipcount']})</p>";
			flush();
		...
		case 4:
			...
			{
				case 10:
					echo "<p>Aktualizacja tabeli użytkowników (4/4)...</p>";
					flush();
					...
					break;
				case 9:
					echo "<p>Aktualizacja tabeli użytkowników (3/4)...</p>";
					flush();
					...
					break;
				case 8:
					echo "<p>Aktualizacja tabeli wątków...</p>";
					flush();
					...
					break;
				case 7:
					echo "<p>Aktualizacja tabeli sesji...</p>";
					flush();
					...
					break;
				case 6:
					echo "<p>Aktualizacja tabeli logów wyszukiwania...</p>";
					flush();
					...
					break;
				case 5:
					echo "<p>Aktualizacja tabeli postów (2/2)...</p>";
					flush();
					...
					break;
				case 4:
					echo "<p>Aktualizacja tabeli logów moderatorów...</p>";
					flush();
					...
					break;
				case 3:
					echo "<p>Aktualizacja tabeli logów wiadomości...</p>";
					flush();
					...
					break;
				case 2:
					echo "<p>Aktualizacja tabeli sesji administratorów...</p>";
					flush();
					...
					break;
				default:
					echo "<p>Aktualizacja tabeli logów administratorów...</p>";
					flush();
			...
			break;
		case 3:
			echo "<p>Aktualizacja tabeli użytkowników (2/4)...</p>";
			flush();
			...
			break;
		case 2:
			echo "<p>Aktualizacja tabeli użytkowników (1/4)...</p>";
			flush();
			...
			break;
		default:
			echo "<p>Aktualizacja tabeli postów (1/2)...</p>";
			flush();
	...
	{
		$contents = "<p><input type=\"hidden\" name=\"iptask\" value=\"{$next_task}\" />{$iptable}{$ipstart}Zakończono. Naciśnij Dalej, aby kontynuować konwersję adresów IP.</p>";
...
function upgrade30_updatetheme()
{
	...
	$output->print_header("Aktualizacja stylów");

	// New default user star
	$contents = "<p>Aktualizacja domyślnej ikony gwiazdki... ";
	$db->update_query("usergroups", array('starimage' => 'images/star.png'), "starimage='images/star.gif'");
	$contents .= "zakończono.</p>";

	$contents .= "<p>Dodawanie nowego arkusza stylów... ";
	...
	$contents .= "<p>Dodawanie porządku wyświetlania do wszystkich arkuszy stylów... ";
	...
	$contents .= "<p>Dodawanie domyślnych kolorów... ";
	...
	$contents .= '<p>Przebudowa pamięci podręcznej oraz zmniejszanie istniejących arkuszy stylów...</p>';

	$num_re_cached = recache_existing_styles();

	$contents .= "Zakończono. {$num_re_cached} arkuszów stylów zostało zaktualizowanych w pamięci podręcznej.";
...
function upgrade30_acppin()
{
	...
	$output->print_header("Dodawanie PINu ACP");

	echo "<p>Dodaliśmy nową możliwość zabezpieczenia forum w MyBB 1.8: opcję ustawienia numeru PIN, który każdorazowo należy wpisać podczas logowania do ACP.<br />\n";
	echo "Jeśli nie chcesz korzystać z tej opcji, po prostu opuść ten krok (pozostaw pole puste). PIN możesz ustawić też w późniejszym czasie (informacje o tym, jak to zrobić, znajdziesz w pomocy).</p>\n";
	echo '<div class="border_wrapper">
			<div class="title">Konfiguracja PINu ACP</div>
			<table class="general" cellspacing="0">
				<tbody>
				<tr>
					<th colspan="2" class="first last">Numer PIN ACP</th>
				</tr>
				<tr class="first">
...
function upgrade30_acppin_submit()
{
	...
	$output->print_header("Zapisywanie pliku konfiguracyjnego");

	$content = "<p>Zapisywanie Twojego PINu (jeśli został wpisany) do pliku config.php... ";

	if(!is_writable(MYBB_ROOT."inc/config.php"))
	{
		$content .= "Błąd (config.php nie jest zapisywalny)";
	}
	else if(isset($config['secret_pin']))
	{
		$content .= "Pominięto (PIN został ustawiony wcześniej)";
	}
	else
	{
